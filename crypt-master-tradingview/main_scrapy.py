...
def generate_csv(a, crypt,delta):
    ...
    for n in config["synth"].get(delta):
        ...
        if n == 1:
            ...
            loop.run_in_executor(None,save_df,df,savename)
            continue
        synth = []
...
async def main():
    ...
    while True:
        ...
        try:
            ...
            # for crypt in crypts:
            #     for delta in deltas:
            for delta,v in config["simple"].items():
                if first> 3 and (now.minute % int(delta)) > 3:
                    continue
                first += 1
                for crypt in v:
                    ...
                    while True:
                        try:
                            ...
                            if "timescale_update" in result:
                                # loop = asyncio.get_event_loop()
                                loop.run_in_executor(None,generate_csv,result,crypt,delta)
        ...
        # else:
        #     print(f"current {start} next time {interval} minute")
        #     time.sleep(60 * interval)
        #
        # if datetime.now().minute % less != 0:
        #     resiual = datetime.now().minute % less
        #     print(f"current time {datetime.now()} next time {less - resiual} minute")
        #     time.sleep((less - resiual) * 60)
        #
        # else:
        #     print(f"current time {datetime.now()} next time {less} minute")
        #     time.sleep(60 * (less))
