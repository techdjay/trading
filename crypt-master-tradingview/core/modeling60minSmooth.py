...
def dataset(select=None, crypts=None):
    for files in glob(DATADIR + "/*"):
        for file in glob(files + "/*"):
            ...
            df = df.iloc[20:,:]
            rows = df.shape[0]
            yield df, file.split(os.sep)[-1].split(".")[0], vote.iloc[-rows:,:], pch.iloc[-rows:,:], actual_pch[-rows:], ma.iloc[-rows:,:], pc.iloc[-rows:]
...
def analysis():
    ...
    gen = dataset(select=synth, crypts=crypts)
    global totalvotes
    temp = {}
    # WARNING = []
    offset = 0
    ...
    for data, name, vote, pch, actual_pch, ma, pc in gen:
        ...
        plt.plot(smoothmadiff, "g-", label="smoothdiff", lw=2,alpha=0.9)
        plt.scatter(x, smoothmadiff,c=ma["smoothma-diff"]>0)
        ...
        plt.scatter(x, temp["pchs"].iloc[-offset:], alpha=0.9, c=np.int8((temp["pchs"]>=0).values),cmap="PRGn", label="pchdot", marker="*")
        # plt.scatter()
