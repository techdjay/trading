    ...
    for files in glob(DATADIR + "/*"):
        for file in glob(files + "/*"):
            ...
            for i in deltas:
                ...
                ch[i] = df[i] - df[i].shift(1)
            df = df[20:]
            rows = df.shape[0]

            yield df, file.split(os.sep)[-1].split(".")[0], vote[-rows:], pch[-rows:], actual_pch[-rows:],ch[-rows:]
...
def analysis():
    ...
    gen = dataset(select=["5"], crypts=["DOGE", "ZEC", "BSV", "XRP"])
    # gen = dataset(select=["30"], crypts=crypts)
    temp = {}
    WARNING = []
    offset = 0
    for data, name, vote, pch, actual_pch ,ch in gen:
        # if "DOGE" not in name:
        #     continue
        ...
        for i in range(data.shape[0] - 1, -1, -1):
            ...
            chs.insert(0, round(ch.iloc[i].mean(), 3))
        ...
        temp = pd.DataFrame({"date": data.index, "votes": votes, "pchs": pchs,"chs":chs})
        # ""
        ...
        plt.plot(temp["chs"],alpha=0.5,label="ch",c="gold")
