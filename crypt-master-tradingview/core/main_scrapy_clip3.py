...
def generate_csv(a, crypt,delta):
    ...
    if len(x) < 50:
        print(f"{filename} less than 50")
        return  [delta,(crypt,)]
    os.makedirs(os.path.join(SAVEDIR, re.match("[A-Z]*", filename).group()), exist_ok=True)
    ...
    for n in config["synth"].get(delta):
        ...
        if clipnd and n in clipnd:
            ...
            if not (t.hour * 60 + t.minute) % (int(n) * int(dlt)) == 0:
                print(f"skip {cname} {n}")
                continue
            # if n >2:
            #     resiual = (t.hour % (n//2))+1
            #     tdf = tdf.drop(axis=1, index=tdf.index[-resiual*2:])
        savename = os.path.join(SAVEDIR,cname , f'{filename}X{n}.csv')
        ...
        try:
            ...
            while q:
                ...
                for crypt in v:
                    ...
                    sendMessage(ws, "create_series", [chart_session, "s1", "s1", "symbol_1", delta, 300])
                    while True:
                        try:
