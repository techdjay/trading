...
def httppost(data):
    # d_order = list(filter(lambda x: any([j in x["name"] for j in config["alarm"]]), data))
    d_order = list(filter(lambda x: any([j in x["name"] for j in config["email"]]) and "15X" not in x["name"], data))
    ...
    for deltaT, crypts in config["simple"].items():
        ...
        for data, name, vote, pch, actual_pch, ma, pc in gen:
            ...
            for ee in indexofemail:
                shifttime = (datetime.strptime(ee[1], "%Y-%m-%d %H:%M:%S") + dt.timedelta(seconds=deltatime * 60)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        print(f"process complete {deltaT}")
        if warns:
