...
def get_historic_price(query_url,json_path,csv_path):
    ...
    try:
        ...
        try:
            ...
            Symbol=parsed['chart']['result'][0]['meta']['symbol']
            ...
            df=pd.DataFrame(list(zip(Symbol,Date,Low,Open,Volume,High,Close,Adjusted_Close)),columns =['symbol','Date','Low','Open','Volume','High','Close','Adjusted Close'])

            if os.path.exists(csv_path+stock_id+'.csv'):
                os.remove(csv_path+stock_id+'.csv')
            df.to_csv(csv_path+stock_id+'.csv', sep=',', index=None)
            print(">>>  One Min data of "+stock_id+" saved")
        
        except:
            print(">>>  1 Min data of "+stock_id+" could not be saved")
        
        return

def main():

	json_path = os.getcwd()+os.sep+".."+os.sep+"1min_data"+os.sep+"json"+os.sep
	csv_path = os.getcwd()+os.sep+".."+os.sep+"1min_data"+os.sep+"csv"+os.sep
	...
	for ticker in df['Ticker']:
	    query_urls.append("https://query1.finance.yahoo.com/v8/finance/chart/"+ticker+"?symbol="+ticker+"&period1=1627526681&period2=9999999999&interval=5m&includePrePost=true&events=div%2Csplit")

	with Pool(processes=10) as pool:
	    pool.starmap(get_historic_price, zip(query_urls, itertools.repeat(json_path), itertools.repeat(csv_path)))

	print("<|>  1min data of all stocks saved")
