...
def on_message(wss, message):
    ...
    if 'timescale_update' in message:
        cs_session, symbolbank, btms = get_list_price_model_in_message(message)
        # Request more history data
        wss.send(generate_message('request_more_data', [cs_session, 's1', max_get_data]))
        logger.info('Data receive: ' + str(len(btms)) + ' data -> ' + symbolbank)
        if btms:
            # Case request more history data
            if len(btms) > 1:
                new_prices = filter_new_prices(btms, symbolbank)
                if new_prices:
...
def on_open(wss):
    ...
    def run():
        for symbolbank in symbols:
            cs = dict_session[symbolbank]['cs']
            wss.send(generate_message('chart_create_session', [cs]))
            if len(symbols) == 1:
                resolve_symbol = "={\"symbolbank\":\"" + symbolbank + "\",\"adjustment\":\"splits\"}"
            else:
                resolve_symbol = "={\"symbolbank\":\"" + symbolbank + "\",\"adjustment\":\"splits\",\"session\":\"extended\"}"
            ...
            logger.info('Send message get data ' + symbolbank + ' successfully')
            time.sleep(1)
...
def generate_message(m, p):
    ...
    params = params.replace("'", '"')
    package_symbols = config.get('symbolbank', 'package_symbols')
...
def get_list_price_model_in_message(message):
    ...
    prices = p[1]['s1']['s']
    result = []
    symbolbank = get_symbolbank_by_cs_session(dict_session, cs_session)
    model = dict_session[symbolbank]['model']
    for price in prices:
        v = price['v']
        bm = model(name=symbolbank, time_frame=convert_time_frame(),
                   time=int(v[0]), open=v[1], high=v[2], low=v[3], close=v[4], vol=v[5])
        result.append(bm)
    return cs_session, symbolbank, result
...
def get_symbol_by_cs_session(dict_symbolbank, cs_session):
    for key in dict_session.keys():
        if dict_symbolbank[key]['cs'] == cs_session:
            return key
...
if __name__ == "__main__":
    ...
    symbols = set(re.sub(' +', '', config.get('socket', 'symbolbank')).split(','))
